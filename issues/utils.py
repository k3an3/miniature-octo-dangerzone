...
Thank you!'''.format(username, text, link)
    ...
    send_mail(subject, body, 'moctod@apalinode.com', [email], fail_silently=False)

def new_issue_email(username, email, link):
    body = '''Hello {0},
...
Thank you!'''.format(username, link)
    ...
    send_mail(subject, body, 'moctod@apalinode.com', [email])
